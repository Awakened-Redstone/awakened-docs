...
<Steps>
  ...
  <Spoiler title={"Adding to your server"}>
    ...
    3. Select OAuth2 on the sidebar
    4. Select `bot` and `application.commands` in *OAuth2 URL Generator* - NOT `applications.commands.permissions.update`
    5. Select `Send messages` in _Bot Permissions_
    6. Copy the generated link on the bottom of the page
    7. Paste in your URL bar
    8. Add the bot to you server (you can do it while it is in there, that will update it's permissions)
  </Spoiler>
