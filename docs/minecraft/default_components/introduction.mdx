...
All items have a unique ID on the following format: `namespace:path`, for example `minecraft:diamond`  
...
The path represents the item, but not always matches the item name, for example, `Block of Diamond` is `diamond_block`
:::
...
**To modify the components of ALL items you must create a file called `all.json` in the `minecraft` namespace**
